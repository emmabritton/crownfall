...
impl PieceRenderer {
    pub fn new() -> Self {
        Self {
            crown_white: IndexedImage::from_file_contents(include_bytes!("../resources/crown_white.ici"))
                .unwrap()
                .0,
            crown_black: IndexedImage::from_file_contents(include_bytes!("../resources/crown_black.ici"))
                .unwrap()
                .0,
            knight_white: IndexedImage::from_file_contents(include_bytes!("../resources/knight_white.ici"))
                .unwrap()
                .0,
            knight_black: IndexedImage::from_file_contents(include_bytes!("../resources/knight_black.ici"))
                .unwrap()
                .0,
            spy_white: IndexedImage::from_file_contents(include_bytes!("../resources/spy_white.ici"))
                .unwrap()
                .0,
            spy_black: IndexedImage::from_file_contents(include_bytes!("../resources/spy_black.ici"))
                .unwrap()
                .0,
        }
    }
    ...
    pub fn render(&self, graphics: &mut Graphics) {
        for y in 0..BOARD_LENGTH {
            for x in 0..BOARD_LENGTH {
                let cell_pos = self.pos + Coord::new((x * CELL_SIZE) as isize, (y * CELL_SIZE) as isize);
                let image = if (x + y) % 2 == 0 { &self.light } else { &self.dark };
